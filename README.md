...
3. Ввод суммы для пополнения. Пользователь указывает сумму, которую хочет перевести на счёт телефона.
4. Запрос состояния платёжного счёта (например, банковского счёта, с которого списываются средства). Система отправляет запрос в  базу данных платёжного счёта, чтобы получить информацию о состоянии счёта и возможности провести операцию.
5. Отправка платежа. Сумма списывается с баланса платежного счёта, новый баланс сохраняется в БД банка.
6. Обновление баланса телефона. Платеж поступает на счет оператора связи, баланс пополняемого счёта пользователя увеличивается на указанную сумму, в БД сохраняются обновлённые данные.
